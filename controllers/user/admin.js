...
const { contentTypes: contentTypesUtils } = require('siapi-utils');
...
const findEntityAndCheckPermissions = async (ability, action, model, id) => {
  const entity = await siapi.query('user', 'users-permissions').findOne({ id });

  if (_.isNil(entity)) {
    throw siapi.errors.notFound();
  }

  const pm = siapi.admin.services.permission.createPermissionsManager({ ability, action, model });

  const roles = _.has(entity, `${CREATED_BY_ATTRIBUTE}.id`)
    ? await siapi.query('role', 'admin').find({ 'users.id': entity[CREATED_BY_ATTRIBUTE].id }, [])
    : [];
...
module.exports = {
  ...
  async create(ctx) {
    ...
    const pm = siapi.admin.services.permission.createPermissionsManager({
      ability: userAbility,
    ...
    const userWithSameUsername = await siapi
    ...
    if (advanced.unique_email) {
      const userWithSameEmail = await siapi
        .query('user', 'users-permissions')
    ...
    try {
      const data = await siapi.plugins['users-permissions'].services.user.add(user);
  ...
  async update(ctx) {
    const advancedConfigs = await siapi
      .store({
        environment: '',
        type: 'plugin',
        name: 'users-permissions',
    ...
    const data = await siapi.plugins['users-permissions'].services.user.edit({ id }, updateData);
